...
const RequestList = () => {
  ...
  useEffect(() => {
    ...
    if (auth.role !== "employee" && auth.role !== "manager") {
      ...
      axios
        ...
        .then((result) => {
          const newData = result.data.data.advanceRequests
            .filter((x) => x.email === email)
            .map((x) => ({
              "Türü": x.advanceType === "individual" ? "Bireysel" : "Kurumsal",
                "Talep Tarihi":formatDate(x.added),
              Tutar: x.advanceAmount,
              "Para Birimi": x.advanceCurrency,
              "Açıklama":x.description,
              "Onaylanma Durumu":capitalizeFirstLetter(x.requestStatus),
              id:x.id
            }));
  ...
  const handleDelete = (id) => {
    console.log(id,auth);
    axios
      .delete(`https://ikprojesitakimiki.azurewebsites.net/api/Request/DeleteAdvanceRequest/${id}`,
        {
        headers: {
          Authorization: `Bearer ${auth.token}`,
        },
